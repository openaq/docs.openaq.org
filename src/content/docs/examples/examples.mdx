...
address them. For more complex solution we recommend using one of the official
API clients or SDKs, see [Libraries](../using-the-api/libraries) for more
information.

:::note Examples on this page will not work as-is. All requests use a
placeholder API Key value, `YOUR-OPENAQ-API-KEY`. This key will not work and
will return an HTTP 401 error, replace with your own API key. :::
