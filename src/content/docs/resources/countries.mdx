...
provider. OpenAQ may not use the same country label as reported by the upstream
provider. OpenAQ uses the Natural Earth 10m dataset for country boundaries, see
the [disclaimer](disclaimers/countries).
...
* id - the unique identifier used within the OpenAQ system for a
country.
